# SEG Decision Log — Research stage

General-grounds conclusions and the composability / SPDX-exchange stream. This is
one of two stage-slices of the SEG decision log; **DEC-IDs are globally unique and
stable across both slices**. Full map: `notes/decision_log_index.md`.

**Holds:** DEC-003 (principle half), DEC-007, DEC-008, DEC-009, DEC-010, DEC-012…DEC-028.
DEC-003 is split — its Python/marker *binding* lives in the development slice
(`development/notes/decision_log.md`); the language-agnostic raw-byte-hashing
*principle* is below. The core-engine DECs (001–006) are in the development slice.
**DEC-011 is intentionally vacant** (the projection-core ADR, `research/notes/seg_adr_projection_core.md`).

**Companion docs:** `research/notes/` (composability_cbd, definition_language, ADR,
prior_art, GAPS, reconciliation, spdx handoff); research prototypes + demos in
`research/prototypes/`.
...
## DEC-003 (principle) — raw-byte hashing, parser-as-locator (language-agnostic)

**Status:** Accepted

> **Split entry.** DEC-003's Python/marker *binding* half lives in the development
> slice (`development/notes/decision_log.md`). This half holds the language-agnostic
> hashing principle.
...
doxygen/sphinx-needs/grammar version churn.

**See also.** DEC-003 (binding) for the Python node fields, marker dialect, and
implementation consequences (development slice); AC-004 is the binding engine
constraint.
