# ============================================================================
# File A — seg_example_v1_baseline.dsl
# The v1 Safety Evidence Graph expressed in the graph-type definition language.
...
#        edge_field(EdgeId, Name, Value) symmetric; for payload edges (unused in v1)
#
...
# sublanguage. node/2 and edge/4 derive naturally from the graph entities;
# node_field/3 is the uniform accessor for the fields a node declares; edge_state/2
# is the sole core-computed integrity primitive; the context tier carries
# non-graph evaluation inputs. Built-in comparisons (e.g. before/2) are
# range-restricted filters — both arguments must be bound by positive literals
# first — so the program stays in the decidable/terminating fragment.
#
# ---- Verdict purity invariant ----
# A SEALED verdict (`satisfied`, ...) may depend only on graph-extensional facts,
# `edge_state`, and SEALED context. No unsealed input — a `tracked` field or live
# (unsealed) context such as wall-clock now() — may sit upstream of a sealed
# verdict, or it would not be reproducible from the proof alone. (This is the
# tracked-field rule and the eval-time pinning, generalized to one statement.)
# Report-only predicates may read anything.
# ============================================================================
...
node TestOutcome {
    ...
    # (edge_state), so the rules drop a stale outcome with no special-casing.
...
# ---- Edge types (the v1 seven) ----
edge refines {                                # child -> parent; acyclic declared (satisfaction recursion)
    from:        Requirement;
    to:          Requirement;
    binds:       true;
    propagates:  true;
...
# edge calls {                                # RESERVED in v1 (not extracted). The canonical
#     from:        Implementation;            # cyclic edge: legitimately loops; no fingerprint
#     to:          Implementation;            # facet in v1 — the case that fractured `strong`.
#     binds:       false;
#     propagates:  true;
# }
...
rules {
    ...
    # REPORT-ONLY signals may read anything (incl. tracked / live context) but must
    # never sit upstream of `satisfied`. (none in the v1 baseline; ADR adds one in File B.)
}