...
2. **Orphan branches for the four dev branches.** `reqs / impl / results / graph`
   (in folders `a-reqs/ b-impl/ c-results/ g-graph/`) are **orphan** (disjoint
   histories, independent SHAs → each `repoXSha` stays meaningful). Rationale:
   research→development is a **maturity re-authoring**,
...
3. **Letter-prefixed folder names** (diverges from DEC-002's bare `A/B/C/G`):
   dirs `a-reqs/ b-impl/ c-results/ g-graph/` (prefixed so they sort in listing
   order), with clean branch names `reqs / impl / results / graph`. The logical
   repo keys `repoA…repoG` (seg.yaml keys, `repoBSha`-style manifest fields) are
   **independent of both folder and branch names** and unchanged.

4. **Decision log: one home per slice, no mirroring.** The dev-binding slice
   (DEC-001…006) is to live physically on branch `impl` (`b-impl/`); the direction slice
...
├── .git             pointer → ./.bare
├── seg.yaml         untracked; repoA→a-reqs repoB→b-impl repoC→c-results repoG→g-graph
├── .premigration/   safety net (pre-migration working tree; retained)
├── research/    [research]  be331de
├── a-reqs/      [reqs]      repoA — requirements
├── b-impl/      [impl]      repoB — implementation + test specs
├── c-results/   [results]   repoC — test outcomes
└── g-graph/     [graph]     repoG — graph state (ships seg.yaml.example + .gitignore)
```
...
- **Decision-log promotion** of DEC-001…006 onto branch `impl` (`b-impl/`) — needs the placement call.
